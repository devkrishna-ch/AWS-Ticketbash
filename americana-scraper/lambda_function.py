...
from americana_scraper import scrape_event
...
def lambda_handler(event, context):
    ...
    try:
        ...
        perf_id = body["event_unique_id"]
        skybox_id = body["event_id"]
        venue_id = int(body.get("venue_id", 0))
        venue_name = body.get("venue_name", "Americana Theatre")
        ...
        try: 
           raw = scrape_event(venue_name, perf_id, evt_date, evt_date, 3)
           logger.info("[lambda_handler] Scrape complete. Raw data length: %d", len(raw) if raw else 0)
        except Exception as scrape_error:
            error_msg = f"Scraping failed: {str(scrape_error)}"
            log_error_to_db(engine,
                venue_name=venue_name,
        ...
        # Check if scraper returned an error status
        if out.get("status") == "error":
            scraper_error_msg = out.get("message", "Unknown scraper error")
            payload = {
                "reason": scraper_error_msg,
                "status": "error",
                "event_id": event_num,
                "venue_id": venue_id,
                "venue_name": venue_name,
            ...
            log_error_to_db(engine, venue_name=venue_name, venue_id=str(venue_id), event_name=evt_name,
                            ...
                            error_details=scraper_error_msg, process_name=process)
