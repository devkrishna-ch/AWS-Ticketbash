...
from bradley_playhouse_scraper import scrape_event
...
def save_eventData_to_db(response_body: str) -> None:
    ...
    logger.info("saved bradley_Events event_data sample: %s", rows[:1])
...
def lambda_handler(event, context):
    ...
    engine = None
    venue_name = "The Bradley Playhouse"
    evt_name = ""
    evt_date = ""
    evt_time = ""
    process = ""
    ...
    try:
        ...
        venue_name = body.get("venue_name", "The Bradley Playhouse")
        ...
        try:
            bradley_Events = scrape_event(event_url, event_unique_id, venue_name)
            try:
                out = json.loads(bradley_Events)
            except json.JSONDecodeError:
                out = ast.literal_eval(bradley_Events)
        ...
        payload = {
            ...
            "venue_name": body.get("venue_name", "The Bradley Playhouse"),  # Get venue name with default
