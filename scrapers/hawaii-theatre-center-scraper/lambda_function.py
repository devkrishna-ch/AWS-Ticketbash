...
from hawai_theatre_center_scraper import scrape_event
...
def save_eventData_to_db(response_body: str) -> None:
    ...
    logger.info("saved hawaii_Events event_data sample: %s", rows[:1])
...
def lambda_handler(event, context):
    ...
    engine = None
    venue_name = "hawaii theater center"
    evt_name = ""
    evt_date = ""
    evt_time = ""
    process = ""
    ...
    try:
        ...
        venue_name = body.get("venue_name", "hawaii theater center")
        ...
        try:
            fetch_Events = scrape_event(event_url,venue_name)
            try:
                out = json.loads(fetch_Events)
            except json.JSONDecodeError:
                out = ast.literal_eval(fetch_Events)
        ...
        payload = {
            ...
            "venue_name": body.get("venue_name", "hawaii theater center"),  # Get venue name with default
